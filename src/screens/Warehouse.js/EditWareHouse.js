...
// export default function EditWareHouse(props, { navigation }) {
//   console.log(props, "363");
//   console.log(props.route.params.data.addData.warehouse_name, "3633");
//   console.log(props?.data?.warehouse_name, "787");
//   const {
//     handleSubmit,
//     formState: { errors },
//     control,
...
//     props?.route?.params?.data?.addData?.email
//   );
//   const [code, setCode] = useState("");
//   const [name, setName] = useState(
//     props?.route?.params?.data?.addData?.warehouse_name
//   );
//   const [phoneNumber, setPhoneNumber] = useState(
//     props?.route?.params?.data?.addData?.phone_number
//   );
//   const [mobileNumber, setMobileNumber] = useState(
//     props?.route?.params?.data?.addData?.mobile_number
//   );
//   const [address, setAddress] = useState(
//     props?.route?.params?.data?.addData?.address
//   );
...
//     const result = await api.getFilter(token, endPoint.edit_warehouse, myJson);
//     if (result.success === "1") {
//       // alert("Register");
...
export default function EditWareHouse(props, { navigation }) {
  ...
  const [wearhouseError, setWearhouseError] = useState();
  const [warehouseId, setWarehouseId] = useState(
    props?.route?.params?.data?.addData?._id
  );
  console.log(warehouseId, "012");
  const [email, setEmail] = useState(
    props?.route?.params?.data?.addData?.email
  );
  const [name, setName] = useState(
  ...
  const [countrykey, setcountrykey] = useState(
    props?.route?.params?.data?.addData?.country
  );
  const [assignUserlabel, setAssignUserLabel] = useState(
    props?.route?.params?.data?.addData?.warehouse_user
  );
  const [assignUserkey, setAssignUserkey] = useState(
    props?.route?.params?.data?.addData?.warehouse_username
  );

  const [area, setArea] = useState("");
  const [areakey, setAreakey] = useState(
    props?.route?.params?.data?.addData?.area
  );
  ...
  const [cityKey, setCityKey] = useState(
    props?.route?.params?.data?.addData?.city
  );
  const [city, setCity] = useState("");

  const [status, setStatus] = useState(
    props?.route?.params?.data?.addData?.status === 1 ? "Active" : "Inactive"
  );
  const [statuskey, setstatuskey] = useState(0);
  ...
  const handleSubmitPress = async (data) => {
    ...
    if (result.success === "1") {
      ...
      console.log(result, "error");
      setWearhouseError(result.message);
      refRBSheet1.current.open();
    }
  };
        ...
        <View
          ...
          <View style={[styles.flexRow, styles.justifyCenter]}>
            <TouchableOpacity
              ...
              //   // refRBSheet2.current.close(),
              // ]}>
              onPress={() => props.navigation.navigate("WareHouseList")}>
              <Text style={[styles.font16, styles.textWhite, styles.letspa35]}>
                Continue
              </Text>
            </TouchableOpacity>

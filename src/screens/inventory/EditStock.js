...
const[typeData,setTypeData] = useState([])
  ...
  const getTypeData = async () => {
    ...
    const result = await api.getType(
      token,
      endPoint.get_Type,
      myJson
    );
...
const filterSKUDATA  =  (product_na) => {
  const filter_prdct = Productsdata.filter((item) => item.product_name === product_na);
  if (filter_prdct){
...
const listDataArray = typeData.map((type, index) => {
  ...
  const onSubmit = async () => {
    ...
    const result = await api.createInventory(token, endPoint.create_Inventory, myJson);
    if (result.success === "1") {
  ...
  return (
    <View style={GlobalStyles.orderContainer}>
       <DateTimePicker
        ...
        minDate={new Date(new Date().setDate(new Date().getDate() - 1))}        onConfirm={handleDatePicked}
      ...
      <View style={GlobalStyles.paymentHeaderView}>
        <View style={GlobalStyles.paymentHeaderPaddingView}>
          <View style={GlobalStyles.changeFlexDirection}>
            ...
            <Text style={GlobalStyles.menuText}> Adjust Inventory</Text>
          </View>
        </View>
      </View>
...
<View style={[styles.marBtm12, styles.wdth100]}>
        ...
        <View style={GlobalStyles.defaultSubmitCancel}>
          <TouchableOpacity onPress={() => navigation.navigate("AddCatalogueScreen")}>

...
const introSlider = (props) => {
  ...
  const renderDefaultButton = (name, label) => {
    label === "Skip"
      ? (content = <Text style={styles.buttonTextSkip}>{label}</Text>)
  ...
  const renderOuterButton = (content, name, onPress) => {
    ...
    return (
      <View style={!props.bottomButton && style}>
        <TouchableOpacity
          onPress={onPress}
          style={props.bottomButton && styles.flexOne}>
          {content}
        </TouchableOpacity>
      </View>
    );
  ...
  const renderSkipButton = () =>
    ...
    renderButton(
      "Skip",
      props.skipLabel,
      () => (props.onSkip ? props.onSkip() : goToSlide(props.data.length - 1)),
  ...
  const renderSkipButtonLabel = () => {
    ...
    return <View style={styles.skipContainer}>{secondaryButton}</View>;
  };
  ...
  const renderPaginationIntro = () => {
    ...
    const secondaryButton =
      (!isFirstSlide && renderPrevButton()) ||
      (!isLastSlide && renderSkipButton());
    const primaryButton = isLastSlide ? renderDoneButton() : renderNextButton();
    return (
  ...
  return (
    ...
    <View style={styles.flexOne}>
      ...
      <FlatList
        ...
        onMomentumScrollEnd={onMomentumScrollEndIntro}
        extraData={extra}
